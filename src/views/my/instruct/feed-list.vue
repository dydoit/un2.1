<template>
  <div class="wrapper">
    ...
    <ul class="list-wrapper">
      <li v-for="item of list" :key="item.id">
        <div class="part1">
          <div>
            <span>
              厂家：{{equipComObj[item.equipmentCompany]}}
            </span>
             <span>
              设备类型：{{equipObj[item.equipmentType]}}
            </span>
             <span>
              提交时间：{{item.applyTime}}
            </span>
            <el-button type="text" @click="showModel(item)">查看详情</el-button>
          </div>
    ...
    <detail-model ref="detail"></detail-model>
...
import DetailModel from './components/detail-model.vue'
...
const options = [
  ...
  export default {
    ...
    methods: {
      async getData() {
        let res = await this.$http.get('/OpsDev/orderAudit/queryOrderAuditList', {
          params: {
            ...this.formData,
      ...
      async repeal(id) {
        this.$confirm('撤销该反馈?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(async() => {
          let res = await this.$http.get('/OpsDev/orderAudit/doOrderAuditRevoke', {
          params: {
              id
            }
          })
